...
(function () {
  ...
  let signalRgbEffectsPromise = null;
  let signalRgbLayoutsPromise = null;
  ...
  function refreshCapabilities() {
    return fetch('/actions/catalog').then((r) => r.json()).then((d) => {
      ...
      if (changed) { scenesPromise = null; sourcesPromise = null; sbActionsPromise = null; sbCodeTriggersPromise = null; sbGlobalsPromise = null; discordChannelsPromise = null; discordSoundsPromise = null; haEntitiesPromise = null; haDomains = null; wlChannelsPromise = null; lightDevicesPromise = null; signalRgbEffectsPromise = null; signalRgbLayoutsPromise = null; }   // config changed → re-fetch the lists
  ...
  function signalRgbLayouts() {
    if (!signalRgbLayoutsPromise) signalRgbLayoutsPromise = fetch('/api/signalrgb/layouts').then((r) => r.json())
      .then((d) => ((d && Array.isArray(d.layouts)) ? d.layouts : []).map((e) => ({ value: e.value, label: e.label })))
      .catch(() => []);
    return signalRgbLayoutsPromise;
  }
  ...
  function open(opts) {
    ...
    scenesPromise = null; sourcesPromise = null; appsPromise = null; storeAppsPromise = null; sbActionsPromise = null; sbCodeTriggersPromise = null; sbGlobalsPromise = null; discordChannelsPromise = null; discordSoundsPromise = null; haEntitiesPromise = null; wlChannelsPromise = null; lightDevicesPromise = null; sdkWidgetsPromise = null; signalRgbEffectsPromise = null; signalRgbLayoutsPromise = null;
    ...
    function signalRgbLayoutPickControl(step, name) {
      const wrap = document.createElement('div');
      const txt = input('text', step.params[name] || '');
      txt.placeholder = t('deck_param_layout');
      const writeTxt = () => { step.params[name] = txt.value; };
      txt.addEventListener('input', writeTxt); txt.addEventListener('change', writeTxt);
      wrap.appendChild(txt);
      signalRgbLayouts().then((items) => {
        if (!items || !items.length) return;
        const sel = document.createElement('select'); sel.className = 'deck-ed-input';
        sel.setAttribute('data-cs-search', '');
        sel.setAttribute('data-cs-search-placeholder', t('deck_param_layout') + '...');
        const cur = step.params[name] || '';
        if (cur && !items.some((it) => it.value === cur)) items = [{ value: cur, label: cur }, ...items];
        items.forEach((it) => { const o = document.createElement('option'); o.value = it.value; o.textContent = it.label; sel.appendChild(o); });
        sel.value = cur || items[0].value;
    ...
    function stepParams(host, step) {
      ...
      spec.params.forEach((p) => {
        ...
        if (p.kind === 'signalRgbLayout') {
          if (step.params[p.name] == null) step.params[p.name] = '';
          f.appendChild(signalRgbLayoutPickControl(step, p.name));
          host.appendChild(f);
          return;
        }
